...
int main(int argc, char** argv) {
  ...
  if ((A_coo.rows != B_coo.rows) || (A_coo.rows != C_coo.rows)) {
    std::cout << "Invalid dimension, M must be equal! " << A_coo.rows
              << " != " << B_coo.rows << " != " << C_coo.rows << std::endl;
    std::exit(0);
  }

  if ((A_coo.cols != B_coo.cols) || (A_coo.cols != C_coo.cols)) {
    std::cout << "Invalid dimension, K must be equal! " << A_coo.cols
              << " != " << B_coo.cols << " != " << C_coo.cols << std::endl;
    std::exit(0);
  }
  ...
  thrust::device_vector<char> B_ranks = {'M', 'K'};
  tensor_t<index_t, value_t, B_coord_t> B("B", B_coo, B_ranks);

  thrust::device_vector<char> C_ranks = {'M', 'K'};
  tensor_t<index_t, value_t, C_coord_t> C("C", C_coo, C_ranks);

  thrust::device_vector<char> Z_ranks = {'M', 'K'};
  matrix_t<value_t> Z_mat(M, K);
  ...
  if (parameters.validate) {
    ...
    thrust::host_vector<char> h_Z_ranks = {'M', 'K'};
    ...
    for (auto& coord : h_coords) {
      ...
      for (std::size_t nz = 0; nz < h_B.nnzs; nz++) {
        if (h_B.coords[nz] == B_coord_t{m, k}) {
          B_val = h_B.values[nz];
          break;
        }
      }
      ...
      for (std::size_t nz = 0; nz < h_C.nnzs; nz++) {
        if (h_C.coords[nz] == C_coord_t{m, k}) {
          C_val = h_C.values[nz];
          break;
        }
      }
