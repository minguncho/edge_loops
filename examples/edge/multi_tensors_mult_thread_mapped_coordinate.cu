  ...
  for (auto tile_idx : config.tiles()) {
    for (auto atom : config.atoms(tile_idx)) {
      ...
      for (std::size_t coord_id = 0; coord_id < Z_num_coords; coord_id++) {
        if (Z_coords[coord_id] == Z_coord_t{m, v}) {
...
int main(int argc, char** argv) {
  ...
  if (parameters.validate) {
    ...
    for (auto& coord : h_coords) {
      ...
      for (std::size_t coord_id = 0; coord_id < Z.coords.size(); coord_id++) {
        if (h_Z.coords[coord_id] == Z_coord_t{m, v}) {
  ...
  std::cout << "multi_tensors_mult_thread_mapped_coordinate," << A_name
            << ".mtx," << B_name << ".mtx," << C_name << ".mtx,M=" << M
            << ",K=" << K << ",N=" << N << ",V=" << V
            << ",time(ms)=" << timer.milliseconds() << std::endl;

  tracker.generate_output<edge_expr_t, expr_coord_t>(
      edge_expr, "multi_tensors_mult_thread_mapped_coordinate");
}
