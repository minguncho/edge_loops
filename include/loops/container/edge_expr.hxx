...
struct edge_expr_t {
  ...
  vector_t<expr_coord_t, space> indices; /// List of coordinates of each rank in the iteration space
  ...
  void expand_iteration_points() {
    ...
    error::throw_if_exception((shared_ranks.size() == 0),
      "expand_iteration_points(): A and B share no shared ranks!\n");

    error::throw_if_exception((shared_ranks.size() != 1),
      "expand_iteration_points(): Current version only supports 1 shared rank!\n");
    ...
    for (auto s_rank : shared_ranks) {
      ...
      for (auto& A_coord : h_A.indices) {
        ...
        auto shared_coords = h_B.find_shared_coords(s_rank, A_coord[s_rank_idx]);
    ...
    indices = vector_t<expr_coord_t, memory_space_t::host>(
      h_indices.begin(), h_indices.end());
  ...
  void partition_coordinate_space(std::vector<std::size_t> part_sizes) {
    error::throw_if_exception((part_sizes.size() != dims.size()),
      "partition_coordinate_space(): Invalid size of part_size()! Not equal to number of unique ranks\n");
    ...
    for (auto& coord : h_indices) {
      ...
      for (int i = dims.size() - 1; i >= 0; --i) {
        std::size_t local_tile_coord = static_cast<std::size_t>(coord[i]) / part_sizes[i];
