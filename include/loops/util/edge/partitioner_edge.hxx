...
class WorkQuark {
 ...
 private:
  ...
  vector_t<vector_t<index_t, space>> indices; // Need map? on storing coordinates
  size_t atom_idx;
}*/
...
class WorkAtom {
 public:
  ...
  __host__ WorkAtom() : quarks(nullptr), num_quarks(0), m(0), k(0), n(0), tile_idx(0) {}
  ...
  __host__ void update_quarks(WorkQuark<index_t>* new_quarks)  { quarks = new_quarks; }
  ...
  __host__ __device__ WorkQuark<index_t>* end() const { return quarks + num_quarks; };
...
class Partitioner {
 public:
  ...
  __host__ Partitioner(vector_t<h_tensor_t, memory_space_t::host> &input_tensors,
  ...
  /*__host__ void partition_atoms_coordinate_space(std::size_t M0, 
    ...
    error::throw_if_exception(((M0 <= 0 || K0 <= 0) || N0 <= 0), 
      "partition_atoms_coordinate_space(): Invalid partition size, cannot be <= 0!\n");

    error::throw_if_exception(((M0 > A.rows || K0 > A.cols) || N0 > B.cols),
      std::string("partition_atoms_coordinate_space(): Invalid partition size, exceeding limit!\n")
      + "  {M0: " + std::to_string(M0) + "} > {A_rows: " + std::to_string(A.rows) + "}\n"
      + "  {K0: " + std::to_string(K0) + "} > {A_cols: " + std::to_string(A.cols) + "}\n"
      + "  {N0: " + std::to_string(N0) + "} > {B_cols: " + std::to_string(B.cols) + "}\n");
    ...
    // Prefix sum for atoms_num_quarks to get starting position of work quark for each atom
    vector_t<std::size_t, memory_space_t::host> offsets(num_atoms + 1, 0);
    ...
    vector_t<WorkAtom<index_t>, memory_space_t::host> reordered_work_quarks(quarks.size());
    vector_t<std::size_t, memory_space_t::host> current_atom_pos = offsets;
    for (std::size_t quark_idx = 0; quark_idx < quarks.size(); quark_idx++) {
      std::size_t atom_idx = quarks[quark_idx].atom_idx;
      std::size_t dest_idx = current_atom_pos[atom_idx]++;
      reordered_work_quarks[dest_idx] = quarks[quark_idx];
    }
    ...
    for (std::size_t atom_idx = 0; atom_idx < num_atoms; atom_idx++) {
      ...
      work_atoms[atom_idx] = WorkAtom<index_t>(&work_quarks[offsets[atom_idx]], 
                                                atoms_num_quarks[atom_idx],
                                                atom_idx_m, atom_idx_k, atom_idx_n);
    }
  }*/
  ...
  /*__host__ void partition_tiles_coordinate_space(std::size_t M1, 
    ...
    error::throw_if_exception(((M1 <= 0 || K1 <= 0) || N1 <= 0), 
      "partition_tiles_coordinate_space(): Invalid partition size, cannot be <= 0!\n");

    error::throw_if_exception(((M1 > num_atoms_m || K1 > num_atoms_k) || N1 > num_atoms_n),
      std::string("partition_tiles_coordinate_space(): Invalid partition size, exceeding limit!\n")
      + "  {M1: " + std::to_string(M1) + "} > {num_atoms_m: " + std::to_string(num_atoms_m) + "}\n"
      + "  {K1: " + std::to_string(K1) + "} > {num_atoms_k: " + std::to_string(num_atoms_k) + "}\n"
      + "  {N1: " + std::to_string(N1) + "} > {num_atoms_n: " + std::to_string(num_atoms_n) + "}\n");
    ...
    // Prefix sum for tiles_num_atoms to get starting position of work atom for each tile
    vector_t<std::size_t, memory_space_t::host> offsets(num_tiles + 1, 0);
    ...
    vector_t<WorkAtom<index_t>, memory_space_t::host> reordered_work_atoms(num_atoms);
    vector_t<std::size_t, memory_space_t::host> current_tile_pos = offsets;
    for (std::size_t atom_idx = 0; atom_idx < num_atoms; atom_idx++) {
      size_t tile_idx = work_atoms[atom_idx].tile_idx;
    ...
    for (std::size_t tile_idx = 0; tile_idx < num_tiles; tile_idx++) {
      ...
      work_tiles[tile_idx] = WorkTile<index_t>(&work_atoms[tiles_offsets[tile_idx]], 
                                                tiles_num_atoms[tile_idx]);
    }
  }*/

  // Partition quarks into atoms in position space (only including nonzero entries)
  /*__host__ void partition_atoms_position_space(std::size_t M0, 
                                               std::size_t K0,
                                               std::size_t N0) {
    // Validate input parameters
    error::throw_if_exception(((M0 <= 0 || K0 <= 0) || N0 <= 0), 
      "partition_atoms_position_space(): Invalid partition size, cannot be <= 0!\n");
    ...
    for (std::size_t b_start = 0; b_start < active_rows.size(); b_start += M0) {
      ...
      vector_t<std::size_t, memory_space_t::host> row_local_offsets(b_end - b_start, 0);
      bool block_has_remaining_quarks = true;
      std::size_t atom_k_idx = 0;

      while (block_has_remaining_quarks) {
        block_has_remaining_quarks = false;
        
        for (std::size_t atom_n_idx = 0; atom_n_idx < B.cols; atom_n_idx++) { // N_COUNT_LIMIT depends on your data depth
          ...
          for (std::size_t i = 0; i < (b_end - b_start); i++) {
            ...
            std::size_t total_in_row = A_csr.offsets[actual_row + 1] - row_start_nnz;

            std::size_t taken = 0;
            // We take K0 * N0 elements for a 3D "volume" atom
            while (taken < (K0 * N0) && row_local_offsets[i] < total_in_row) {
              current_atom_indices.push_back(row_start_nnz + row_local_offsets[i]);
              row_local_offsets[i]++;
              taken++;
            }
  ...
  // Partition atoms into tiles in position space (only including nonzero entries)
  /*__host__ void partition_tiles_position_space(std::size_t M1, std::size_t K1) {
    ...
    error::throw_if_exception((M1 <= 0 || K1 <= 0), 
      "partition_tiles_position_space(): Invalid size of tile, cannot be <= 0!\n");
    ...
    vector_t<vector_t<std::size_t, memory_space_t::host>, memory_space_t::host> atom_grid(num_atoms_x, 
    ...
    for (std::size_t atom_id = 0; atom_id < num_atoms; atom_id++) {
      atom_grid[work_atoms[atom_id].get_x_idx()][work_atoms[atom_id].get_y_idx()] = atom_id;
    }
  ...
  /*__host__ void partition_atoms_position_space_flatten(std::size_t nnzs_per_atom) {
    
    // Validate input parameters
    error::throw_if_exception((nnzs_per_atom == 0), 
      "partition_atoms_position_space_flatten(): nnzs_per_atom cannot be zero!\n");

    error::throw_if_exception((nnzs_per_atom > A.nnzs), 
      "partition_atoms_position_space_flatten(): nnzs_per_atom cannot be greater than NNZ of A!\n");
  ...
  /*__host__ void partition_tiles_position_space_flatten(std::size_t num_atoms_per_tile) {

    error::throw_if_exception(!atoms_partitioned, 
      "partition_tiles_position_space_flatten(): Need to partition work atoms first.\n");

    error::throw_if_exception(!rank_flattened, 
      "partition_tiles_position_space_flatten(): Need to partition work atoms using flatten method\n");
  ...
  /*__host__ void prepare_gpu() {
    ...
    for (size_t atom_idx = 0; atom_idx < num_atoms; atom_idx++) {
      temp_atoms[atom_idx].update_quarks(d_quarks_ptr + (work_atoms[atom_idx].get_quarks() - &quarks[0]));
    }
  ...
  /*__host__ vector_t<WorkTile<index_t>>& get_work_tiles() { return d_work_tiles; }
